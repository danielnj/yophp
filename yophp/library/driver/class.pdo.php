...
class MysqlPdo {
	...
	/**
	 * 构造函数，
	 * @param $dbconfig 数据库连接相关信息，array('ServerName', 'UserName',
  'Password', 'DefaultDb', 'DB_Port', 'DB_TYPE')
	 */
	public function __construct($dbConfig = '') {
		if (! class_exists ( 'PDO' ))
			self::throw_exception ( "不支持:PDO" );
			//若没有传输任何参数，则使用默认的数据定义
		if (! is_array ( $dbConfig )) {
			$dbConfig = array ('hostname' => 'localhost', 'username' => 'root', 'password' => '', 'database' => 'passport', 'hostport' => 3306, 'dbms' => '', 'dsn' => 'mysql:host=localhost;dbname=passport' );
		}
  ...
  TABLE_SCHEMA='" . DB_NAME . "'";
		...
		$info = array ();
		foreach ( $result as $key => $val ) {
			$info [$val ['COLUMN_NAME']] = array ('postion' => $val ['ORDINAL_POSITION'], 'name' => $val ['COLUMN_NAME'], 'type' => $val ['COLUMN_TYPE'], 'd_type' => $val ['DATA_TYPE'], 'length' => $val ['MAXCHAR'], 'notnull' => (strtolower ( $val ['IS_NULLABLE'] ) == "no"), 'default' => $val ['COLUMN_DEFAULT'], 'primary' => (strtolower ( $val ['COLUMN_KEY'] ) == 'pri'), 'autoInc' => (strtolower ( $val ['EXTRA'] ) == 'auto_increment'), 'comment' => $val ['COLUMN_COMMENT'] );
		}
	...
	static function haveErrorThrowException() {
		...
		if (count ( $arrError ) > 1) { // 有错误信息
			//$this->rollback();
			self::$error = $arrError [2] . "<br/><br/> [SQL] :
" . self::$queryStr;
		...
		//主要针对execute()方法抛出异常
		if (self::$queryStr == '')
			self::throw_exception ( 'Query was
  empty<br/><br/>[SQL] :' );
  ...
  .* INTO|COPY|ALTER|GRANT|REVOKE|LOCK|UNLOCK';
	...
	static function throw_exception($err) {
		echo '<div style="width:500px;background-color:#CDCDCD;
  color:#A00;font-size:14px;  border:1px #D40000 solid;
  margin:2px;padding:6px;">ERROR:' . $err . '</div>';
	}
